...
if ($Path) {
    ...
    ForEach($Episode in $Episodes) {
        ffmpeg -n -ss 00:01:00 -i $Episode.FullName -frames:v 1 -vf "scale=-1:240" $($Episode.FullName + ".jpg")
        ffmpeg -n -ss 00:01:00 -i $Episode.FullName -loop 0 -vf "select=not(mod(n\, 2880)), scale=-1:240, setpts=(1/120)*PTS" $($Episode.FullName + ".webp")
    }
} else {
